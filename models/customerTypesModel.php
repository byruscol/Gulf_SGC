...
class customerTypes extends DBManagerModel{
   
    public function getList($params = array()){
        $entity = $this->entity();
        $start = $params["limit"] * $params["page"] - $params["limit"];
        $query = "SELECT  `customerTypeId`, `customerType`
    ...
    public function edit(){
        $this->updateRecord($this->entity(), $_POST, array("customerTypeId" => $_POST["customerTypeId"])/*, array("columnValidateEdit" => "assigned_user_id")*/);
    }
    public function del(){
        $this->delRecord($this->entity(), array("customerTypeId" => $_POST["id"])/*, array("columnValidateEdit" => "assigned_user_id")*/);
    }
    ...
    public function entity($CRUD = array())
    {
        ...
        $data = array(
                        "tableName" => $this->pluginPrefix."customerTypes"
                        //,"columnValidateEdit" => "assigned_user_id"
                        ,"entityConfig" => $CRUD
                        ,"atributes" => array(
                            "customerTypeId" => array("type" => "int", "PK" => 0, "required" => false, "readOnly" => true, "autoIncrement" => true)
                            ,"customerType" => array("type" => "varchar","label" => "tipo_cliente_c", "required" => true)
                        )
                    );
            return $data;
    }
