...
class sources extends DBManagerModel{
   
    public function getList($params = array()){
        $entity = $this->entity();
        $start = $params["limit"] * $params["page"] - $params["limit"];
        $query = "SELECT  `sourceId`, `source`
    ...
    public function edit(){
        $this->updateRecord($this->entity(), $_POST, array("sourceId" => $_POST["sourceId"])/*, array("columnValidateEdit" => "assigned_user_id")*/);
    }
    public function del(){
        $this->delRecord($this->entity(), array("sourceId" => $_POST["id"])/*, array("columnValidateEdit" => "assigned_user_id")*/);
    }
    ...
    public function entity($CRUD = array())
    {
        ...
        $data = array(
                        "tableName" => $this->pluginPrefix."sources"
                        //,"columnValidateEdit" => "assigned_user_id"
                        ,"entityConfig" => $CRUD
                        ,"atributes" => array(
                            "sourceId" => array("type" => "int", "PK" => 0, "required" => false, "readOnly" => true, "autoIncrement" => true)
                            ,"source" => array("type" => "varchar","label" => "fuentenc", "required" => true)
                        )
                    );
            return $data;
    }
