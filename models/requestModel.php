...
class request extends DBManagerModel{
    ...
    public function getList($params = array()){
        ...
        $query = "SELECT  `nonConformityId`, `name`, `description` 
                            ...
                            , `gestion`, `tipo_cliente_c`
                          FROM ".$entity["tableName"]."
                          WHERE `deleted` = 0 AND clasificacion_nc_c = 3";
    ...
    public function add(){
        $this->addRecord($this->entity(), $_POST, array("clasificacion_nc_c" => 3, "date_entered" => date("Y-m-d H:i:s"), "created_by" => $this->currentUser->ID));
        $this->sendAssignedMail($this->DBOper["data"]["assigned_user_id"], $this->LastId, "request");
    }
    public function edit(){
        ...
        if(array_key_exists("assigned_user_id", $this->DBOper["data"])){
            $this->sendAssignedMail($this->DBOper["data"]["assigned_user_id"], $_POST["id"], "task");
        }
    }
    public function del(){
    ...
    public function detail($params = array()){
        ...
        $query = "SELECT n.`nonConformityId` , n.`name` , n.`description` , `status` `estadonc` 
                        ...
                        , m.`management` `gestion`, customerType `tipo_cliente_c`
