...
class generalities extends DBManagerModel{
   
    public function getList($params = array()){
        $entity = $this->entity();
        $start = $params["limit"] * $params["page"] - $params["limit"];
        $query = "SELECT  `generalityId`, `generality`
    ...
    public function edit(){
        $this->updateRecord($this->entity(), $_POST, array("generalityId" => $_POST["generalityId"])/*, array("columnValidateEdit" => "assigned_user_id")*/);
    }
    public function del(){
        $this->delRecord($this->entity(), array("generalityId" => $_POST["id"])/*, array("columnValidateEdit" => "assigned_user_id")*/);
    }
    ...
    public function entity($CRUD = array())
    {
        ...
        $data = array(
                        "tableName" => $this->pluginPrefix."generalities"
                        //,"columnValidateEdit" => "assigned_user_id"
                        ,"entityConfig" => $CRUD
                        ,"atributes" => array(
                            "generalityId" => array("type" => "int", "PK" => 0, "required" => false, "readOnly" => true, "autoIncrement" => true)
                            ,"generality" => array("type" => "varchar","label" => "generalidadnc", "required" => true)
                        )
                    );
            return $data;
    }
