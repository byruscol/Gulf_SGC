...
class classifications extends DBManagerModel{
   
    public function getList($params = array()){
        $entity = $this->entity();
        $start = $params["limit"] * $params["page"] - $params["limit"];
        $query = "SELECT  `classificationId`, `classification`
    ...
    public function edit(){
        $this->updateRecord($this->entity(), $_POST, array("classificationId" => $_POST["classificationId"])/*, array("columnValidateEdit" => "assigned_user_id")*/);
    }
    public function del(){
        $this->delRecord($this->entity(), array("classificationId" => $_POST["id"])/*, array("columnValidateEdit" => "assigned_user_id")*/);
    }
    ...
    public function entity($CRUD = array())
    {
        ...
        $data = array(
                        "tableName" => $this->pluginPrefix."classifications"
                        //,"columnValidateEdit" => "assigned_user_id"
                        ,"entityConfig" => $CRUD
                        ,"atributes" => array(
                            "classificationId" => array("type" => "int", "PK" => 0, "required" => false, "readOnly" => true, "autoIncrement" => true)
                            ,"classification" => array("type" => "varchar","label" => "clasificacion_nc_c", "required" => true)
                        )
                    );
            return $data;
    }
