...
class managements extends DBManagerModel{
   
    public function getList($params = array()){
        $entity = $this->entity();
        $start = $params["limit"] * $params["page"] - $params["limit"];
        $query = "SELECT  `managementId`, `management`
    ...
    public function edit(){
        $this->updateRecord($this->entity(), $_POST, array("managementId" => $_POST["managementId"])/*, array("columnValidateEdit" => "assigned_user_id")*/);
    }
    public function del(){
        $this->delRecord($this->entity(), array("managementId" => $_POST["id"])/*, array("columnValidateEdit" => "assigned_user_id")*/);
    }
    ...
    public function entity($CRUD = array())
    {
        ...
        $data = array(
                        "tableName" => $this->pluginPrefix."managements"
                        //,"columnValidateEdit" => "assigned_user_id"
                        ,"entityConfig" => $CRUD
                        ,"atributes" => array(
                            "managementId" => array("type" => "int", "PK" => 0, "required" => false, "readOnly" => true, "autoIncrement" => true)
                            ,"management" => array("type" => "varchar","label" => "gestionsa", "required" => true)
                        )
                    );
            return $data;
    }
